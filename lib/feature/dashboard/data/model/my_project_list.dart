import 'package:portfolioapp/core/utils/app_images.dart';

import '../../../../core/utils/app_text.dart';
import 'my_project_model.dart';

List<MyProjectModel> myProjectList = [
  MyProjectModel(
    title: AppText.thePerfectBodyApp,
    imagePath: AppImages.perfectBody,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: 'https://play.google.com/store/apps/details?id=com.t5x.fitdiet',
  ),
  MyProjectModel(
    title: AppText.dashboardApp,
    imagePath: AppImages.dashboardApp,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: AppText.dashboardCourseGithub,
  ),
  MyProjectModel(
    title: AppText.newsApp,
    imagePath: AppImages.newsApp,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: AppText.newsAppGithub,
  ),
  MyProjectModel(
    title: AppText.toDoApp,
    imagePath: AppImages.todoApp,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: AppText.toDoAppGithub,
  ),
  MyProjectModel(
    title: AppText.bookApp,
    imagePath: AppImages.booklyApp,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: AppText.booklyAppGithub,
  ),
  MyProjectModel(
      title: AppText.quizApp,
      imagePath: AppImages.quizApp,
      onTap: () {},
      videoPath: '',
      codeInGithubPath: AppText.quizAppGithub),
  MyProjectModel(
    title: AppText.cafeApp,
    imagePath: AppImages.cafeApp,
    onTap: () {},
    videoPath: "",
    codeInGithubPath: AppText.flowerCafeAppGithub,
  ),
  MyProjectModel(
    title: AppText.currenciesApp,
    imagePath: AppImages.currenciesApp,
    onTap: () {},
    videoPath: '',
    codeInGithubPath: AppText.currenciesAppGithub,
  ),
];
